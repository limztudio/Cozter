...
SUMMARY_PROMPT = (
    ...
    "Pure text-summarization: no tools, shell, file reads, or fetches."
    " Work from the text below; emit plain text.\n\n"
    "=== SCRATCH SUMMARY ===\n"
    "Concise abstract REPLACING raw history — thorough enough to continue"
    " seamlessly.\n"
    "The reader always sees [Long-term Memory] too, so never repeat it."
    " Ephemeral only: recent work, current state, open commitments,"
    " in-progress decisions, errors.\n"
    "Capture: what was done and why, paths touched + change nature,"
    " concrete results/errors, done/in-progress/blocked state, open"
    " commitments not yet long-term.\n"
    "Drop: greetings, filler, restatements, redirected explorations,"
    " superseded output, anything already long-term.\n"
    "Topic-grouped prose, 80-200 words. Specific (names, paths, values),"
    " not generic.\n\n"
    "=== LONG-TERM MEMORY ===\n"
    "Durable facts only: preferences, architecture, invariants, stable"
    " project facts, hard constraints. ONE self-contained sentence each.\n"
    "Output the COMPLETE new list in [LONG_TERM] markers (REPLACES the"
    " old list): carry forward what's true, merge similars, drop"
    " wrong/summary-redundant, add new facts."
    " SHORT: 5-15 items, max 30.\n\n"
    "=== TITLE ===\n"
    "Session name: 3-7 words, Title Case, no punctuation/quotes."
    " Dominant topic, e.g. 'Refactor Schedule Storage'.\n\n"
    "=== OUTPUT FORMAT ===\n"
    "All three sections; empty body ok, markers required.\n\n"
    "[SUMMARY]\n"
