...
CONSOLIDATE_PROMPT = (
    ...
    "Pure text-merge: no tools/shell/reads/fetches; work from the input,"
    " emit plain text.\n\n"
    "Goal: canonical cross-session facts, and retire colony items no"
    " session uses.\n\n"
    "=== INPUT ===\n"
    "Current colony list, then per session a 'Session: <name>' header +"
    " [SESSION:<id>] block of its long-term items. Name + items together"
    " judge whether a colony item is still relevant.\n\n"
    "=== TASK ===\n"
    "PROMOTE to colony:\n"
    "- An item in 2+ sessions (verbatim or paraphrase).\n"
    "- An existing colony item whose topic still appears in >=1 session"
    " (name or items).\n\n"
    "KEEP session-local: items specific to one session.\n\n"
    "MERGE near-duplicates into one canonical sentence.\n\n"
    "PRUNE:\n"
    "- Colony items no session represents (colony is cross-session"
    " knowledge; dead topics retire).\n"
    "- Wrong or contradicted items.\n\n"
    "ONE self-contained sentence per item.\n\n"
    "=== OUTPUT FORMAT ===\n"
    "One [COLONY] block, then one [SESSION:<id>] block per input session"
    " (empty ok). Same <id>s, one bullet per item, no 'Session:' headers.\n\n"
    "[COLONY]\n"
    "- <colony item 1>\n"
    "- <colony item 2>\n"
    "[/COLONY]\n\n"
